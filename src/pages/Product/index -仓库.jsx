...
const ProductManagementPage = memo(() => {
  ...
  const [bomSearchTerms, setBomSearchTerms] = useState([]);
  ...
  const openBomModal = async (product) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setBomSearchTerms(new Array(items.length).fill(''));
      } else {
        setBomItems([]);
        setBomSearchTerms([]);
      }
  ...
  const addBomItem = () => {
    ...
    setBomSearchTerms([...bomSearchTerms, '']);
  };
  ...
  const removeBomItem = (index) => {
    ...
    setBomSearchTerms(bomSearchTerms.filter((_, i) => i !== index));
  };
  ...
  const filteredMaterials = materials.filter(m =>
    (`${m.materialCode || m.material_code || ''} ${m.name || ''} ${m.spec || ''}`).toLowerCase().includes(materialSearch.toLowerCase())
  );
          ...
          <>
            ...
            <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                <input
                  ...
                  placeholder="输入编码/名称/规格筛选物料..."
                  style={{ padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '12px', minWidth: '220px' }}
                />
            ...
            {materialSearch && (
              <div style={{ marginBottom: '10px', fontSize: '12px', color: '#64748b' }}>
                已筛选：{filteredMaterials.length} 个物料
              </div>
            )}
            ...
            {bomItems.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '300px', overflowY: 'auto' }}>
                {bomItems.map((item, idx) => {
                  const rowSearch = (bomSearchTerms[idx] || materialSearch || '').toLowerCase();
                  const rowMaterials = materials.filter(m =>
                    (`${m.materialCode || m.material_code || ''} ${m.name || ''} ${m.spec || ''}`).toLowerCase().includes(rowSearch)
                  );
                  return (
                  <div key={idx} style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '12px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <div style={{ flex: 3 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px', gap: '8px' }}>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#64748b' }}>物料 #{idx + 1}</label>
                        <input
                          type="text"
                          value={bomSearchTerms[idx] || ''}
                          onChange={(e) => {
                            const next = [...bomSearchTerms];
                            next[idx] = e.target.value;
                            setBomSearchTerms(next);
                          }}
                          placeholder="输入编码/名称筛选"
                          style={{ padding: '6px 8px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '11px', minWidth: '160px' }}
                        />
                      </div>
                      <select value={item.materialId} onChange={e => updateBomItem(idx, 'materialId', e.target.value)}
                        ...
                        {rowMaterials.length === 0 && <option value="" disabled>无匹配物料</option>}
                        {rowMaterials.map(m => (<option key={m.id} value={m.id}>{m.materialCode || m.material_code} - {m.name} {m.spec ? `(${m.spec})` : ''}</option>))}
                      </select>
                    </div>
                    <div style={{ flex: 1 }}>
